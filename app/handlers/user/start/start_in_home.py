...
import app.keyboards.main_buttons as kb_start_in_home
...
async def start_in_home(message: Message):
    ...
    if await user_yes_no(message.from_user.id):
        await message.answer_photo(photo=photo_id, caption=text,
                                reply_markup=kb_start_in_home.start_in_home_not_reg) # Если не зареган пользователь, ему отправляем с кнопкой регистрации

    else:
        await message.answer_photo(photo=photo_id, caption=text, 
                                reply_markup=kb_start_in_home.start_in_home)
...
async def back_main_menu_in_home(callback: CallbackQuery):
    ...
    if await user_yes_no(callback.from_user.id):
        await callback.message.edit_media(media,
                                        reply_markup=kb_start_in_home.start_in_home_not_reg)
    else:
        await callback.message.edit_media(media, reply_markup=kb_start_in_home.start_in_home)
